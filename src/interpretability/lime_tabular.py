...
from src.interpretability import lime_base
...
class LimeTabularExplainer(object):
    ...
    def explain_instance(self,
        ...
        if sp.sparse.issparse(data):
            ...
            scaled_data = data.multiply(1.0 / self.scaler.scale_)
        ...
        for i in self.categorical_features:
            ...
            if sp.sparse.issparse(data_row):
                if i in feature_indexes:
                    values[list(feature_indexes).index(i)] = 'True'
            else:
                values[i] = 'True'
        ...
        if self.discretizer is not None:
            ...
            if sp.sparse.issparse(data_row):
                data_row = data_row.toarray().ravel()
    ...
    def __data_inverse(self,
        ...
        if sp.sparse.issparse(data_row):
            ...
            if is_sparse:
                # Perturb only the discretized numerical values and non-zero categorical values
                non_zero_indexes = data_row.nonzero()[1]
                if self.discretizer is not None:
                    for idx in self.discretizer.names.keys():
                        if idx not in non_zero_indexes:
                            non_zero_indexes = np.insert(non_zero_indexes, 0, idx)
                    non_zero_indexes.sort()
        ...
        if self.discretizer is not None:
            if sp.sparse.issparse(data_row):
                first_row = data_row.toarray()
                first_row = sp.sparse.csr_matrix(self.discretizer.discretize(first_row))
            else:
        ...
        for column in categorical_features:
            ...
            if is_sparse:
                if column in non_zero_indexes:
                    inverse_column = self.random_state.choice(values, size=num_samples,
                                                              replace=True, p=freqs)
                else:
                    inverse_column = np.zeros((num_samples))
                binary_column = (inverse_column == first_row[0, column]).astype(int)
            else:
                inverse_column = self.random_state.choice(values, size=num_samples,
                                                          replace=True, p=freqs)
